            ...
            switch (opcao) {
                ...
                case 3:{
                    if (primeiroVeiculo == NULL) {
                        printf("Não há veículos registrados. Adicione pelo menos um veículo antes de gerar o arquivo.\n");
                    } else {
                        gerarTxt();
                        system("notepad bloco_eu_sozinho.txt");
                    }
                    system("pause");
                    break;
                }
    ...
    if (primeiroVeiculo == NULL){
        ...
        while (getchar() != '\n');  // Limpar buffer
         ...
         if (posicaoVeiculo <=0 || posicaoVeiculo > totalVeiculos) {
            ...
            printf("\nO veiculo foi editado com sucesso.\n\n");
        }
    }
}
        ...
        while (atualVeiculo != NULL) {
            ...
            if (atualVeiculo->listaPessoa == NULL) {
                ...
                while (ponteiroPessoaTemp != NULL) {
                    printf("\n\t- %s, CPF: %s.", ponteiroPessoaTemp->nome, ponteiroPessoaTemp->cpf);
...
void trocarPessoa() {
    ...
    int totalVeiculos = contadorVeiculos();
    int posicaoVeiculo = 0;
    printf("\nDigite o número do veículo onde se encontra a pessoa que você deseja trocar: ");
    scanf("%d", &posicaoVeiculo);
    while (getchar() != '\n');
    ...
    struct veiculos *atualVeiculo = primeiroVeiculo;
    for (int i = 1; i < posicaoVeiculo; i++) {
        atualVeiculo = atualVeiculo->proximoVeiculo;
    }


    if (atualVeiculo->listaPessoa == NULL) {
        printf("O veículo selecionado não possui pessoas! \n");
        return;
    }

    char pessoaBuscada[TAM_NOME];
    printf("\nDigite o nome da pessoa que você deseja retirar: ");
    fgets(pessoaBuscada, TAM_NOME, stdin);
    pessoaBuscada[strcspn(pessoaBuscada, "\n")] = '\0';

    pessoa *pessoaAntecessor = NULL;
    pessoa *pessoaAtual = atualVeiculo->listaPessoa;


    while (pessoaAtual != NULL && strcmp(pessoaAtual->nome, pessoaBuscada) != 0) {
        pessoaAntecessor = pessoaAtual;
        pessoaAtual = pessoaAtual->proximaPessoa;
    }

    if (pessoaAtual == NULL) {
        printf("\nA pessoa %s não foi encontrada no veículo.\n", pessoaBuscada);
        return;
    }


    if (pessoaAntecessor == NULL) {

        atualVeiculo->listaPessoa = pessoaAtual->proximaPessoa;
    } else {
        pessoaAntecessor->proximaPessoa = pessoaAtual->proximaPessoa;
    }


    int numeroVeiculoDestino = 0;
    printf("\nDigite o número do veículo onde a pessoa vai ficar: ");
    scanf("%d", &numeroVeiculoDestino);
    while (getchar() != '\n');


    struct veiculos *veiculoDestino = primeiroVeiculo;
    for (int i = 1; i < numeroVeiculoDestino && veiculoDestino != NULL; i++) {
        veiculoDestino = veiculoDestino->proximoVeiculo;
    }

    if (veiculoDestino == NULL) {
        printf("\nO veículo com o número %d não existe!\n", numeroVeiculoDestino);

        if (pessoaAntecessor == NULL) {

            atualVeiculo->listaPessoa = pessoaAtual;
        } else {
            pessoaAntecessor->proximaPessoa = pessoaAtual;
        }
        pessoaAtual->proximaPessoa = NULL;
        return;
    }


    if (veiculoDestino->listaPessoa == NULL) {
        veiculoDestino->listaPessoa = pessoaAtual;
        pessoaAtual->proximaPessoa = NULL;
    } else {
        pessoaAtual->proximaPessoa = veiculoDestino->listaPessoa;
        veiculoDestino->listaPessoa = pessoaAtual;
    }

    printf("\nA pessoa %s foi transferida para o veículo número %d.\n", pessoaBuscada, numeroVeiculoDestino);
}
...
void gerarTxt(){
    FILE *arquivo = fopen("bloco_eu_sozinho.txt", "w");
    if (arquivo == NULL) {
    ...
    while (atualVeiculo != NULL) {
        ...
        fprintf(arquivo, "Tipo de Veículo: %s\n", atualVeiculo->tipo);
        fprintf(arquivo, "Placa do Veículo: %s\n", atualVeiculo->placa);

        fprintf(arquivo, "Pessoas no veículo: ");
        if (atualVeiculo->listaPessoa == NULL) {
            fprintf(arquivo, "Não há pessoas associadas a esse veículo.\n");
        } else {
            pessoa *ponteiroPessoaTemp = atualVeiculo->listaPessoa;
            while (ponteiroPessoaTemp != NULL) {
